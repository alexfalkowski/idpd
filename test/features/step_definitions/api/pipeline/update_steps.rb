...
When('we try to update an nonexistent pipeline') do
  ...
  @response = Idpd.http.update_pipeline(10, pipeline.to_json, opts)
end
...
When('we try to update an invalid pipeline') do
  ...
  @response = Idpd.http.update_pipeline(3, pipeline.to_json, opts)
end
...
When('we try to update with an invalid id') do
  ...
  @response = Idpd.http.update_pipeline(0, pipeline.to_json, opts)
end
...
When('we update the simple pipeline') do
  ...
  @response = Idpd.http.update_pipeline(@id, pipeline.to_json, opts)
end

When('we try update a pipeline with a bad payload') do
  @id = JSON.parse(@response.body)['pipeline']['id'].to_i
  @response = Idpd.http.update_pipeline(@id, Base64.encode64('test'), opts)
end
